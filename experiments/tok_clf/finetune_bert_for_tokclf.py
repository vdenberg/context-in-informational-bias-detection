...
parser.add_argument('-lr', '--lr', type=float, default=2e-5) #5e-5, 3e-5, 2e-5
parser.add_argument('-bs', '--bs', type=int, default=16) #16, 21
parser.add_argument('-sv', '--sv', type=int, default=135) #16, 21
parser.add_argument('-fold', '--fold', type=str, default=None) #16, 21
args = parser.parse_args()

N_EPS = args.n_epochs
models = [args.model] if args.model else ['bert']
seeds = [args.sv] if args.sv else [21, 49]  # [49, 6, 34]
...
TASK_NAME = 'tok_clf_bert'
TASK = 'tok_clf'
FEAT_DIR = f'data/inputs/tok_clf/features_for_bert'
...
if __name__ == '__main__':
    ...
    for MODEL in models:
        BERT_MODEL = model_mapping[MODEL]

        for SEED in seeds: #132, 281,  #tok_clf: [132, 281, 45362], sent_clf: 231 (redo this one, tokclf overwrote some), 26354, 182,
            if SEED == 0:
            ...
            for BATCH_SIZE in bss:
                ...
                for LEARNING_RATE in lrs:
                    ...
                    for fold_name in folds:
                        ...
                        best_model = BertForTokenClassification.from_pretrained(os.path.join(CHECKPOINT_DIR, name),
                                                                                num_labels=NUM_LABELS,
                                                                                output_hidden_states=True,
                                                                                output_attentions=False)
